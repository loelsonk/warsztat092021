...
class TypeGraphQLVisitor extends typescript.TsVisitor {
    constructor(schema, pluginConfig, additionalConfig = {}) {
        ...
        autoBind(this);
        this.typescriptVisitor = new typescript.TsVisitor(schema, pluginConfig, additionalConfig);
        const enumNames = Object.values(schema.getTypeMap())
            .map(type => (type instanceof graphql.GraphQLEnumType ? type.name : undefined))
            .filter(t => t);
        this.setArgumentsTransformer(new typescript.TypeScriptOperationVariablesToObject(this.scalars, this.convertName, this.config.avoidOptionals, this.config.immutableTypes, null, enumNames, this.config.enumPrefix, this.config.enumValues));
    ...
    FieldDefinition(node, key, parent, path, ancestors) {
        ...
        const decorator = '\n' +
            visitorPluginCommon.indent(`@TypeGraphQL.${fieldDecorator}(type => ${type.isArray ? `[${type.type}]` : type.type}${formatDecoratorOptions(decoratorOptions, false)})`) +
            '\n';
        typeString = this.fixDecorator(type, typeString);
        return decorator + visitorPluginCommon.indent(`${this.config.immutableTypes ? 'readonly ' : ''}${node.name}!: ${typeString};`);
    }
    InputValueDefinition(node, key, parent, path, ancestors) {
        ...
        const decorator = '\n' +
            visitorPluginCommon.indent(`@TypeGraphQL.${fieldDecorator}(type => ${type.isArray ? `[${typeGraphQLType}]` : typeGraphQLType}${formatDecoratorOptions(decoratorOptions, false)})`) +
            '\n';
